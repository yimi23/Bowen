...
# Import academic engines
try:
    from engines.syllabus_parser import SyllabusParser
    from engines.manual_academic import ManualAcademic
    from engines.backup_manager import BackupManager
    from engines.outlook_connector import OutlookConnector
    ACADEMIC_ENGINES_AVAILABLE = True
except ImportError as e:
    print(f"Warning: Academic engines not available: {e}")
    ACADEMIC_ENGINES_AVAILABLE = False
...
class ConversationalInterface:
    ...
    def __init__(self):
        ...
        # Initialize academic engines
        if ACADEMIC_ENGINES_AVAILABLE:
            print("🎓 Loading academic engines...")
            
            # Academic features
            self.syllabus_parser = SyllabusParser()
            self.manual_academic = ManualAcademic()
            self.backup = BackupManager()
            
            # Email integration
            try:
                self.outlook = OutlookConnector()
                print("✅ Outlook integration loaded")
            except Exception as e:
                print(f"⚠️ Outlook integration failed: {e}")
                self.outlook = None
            
            print("✅ Academic assistant features loaded")
        else:
            print("⚠️ Academic engines not available")
            self.syllabus_parser = None
            self.manual_academic = None
            self.backup = None
            self.outlook = None
    ...
    def _show_capabilities_summary(self):
        ...
        print("\n🎓 **ACADEMIC ASSISTANT:**")
        if ACADEMIC_ENGINES_AVAILABLE:
            print("✅ Syllabus Parsing - Extracts deadlines from PDFs")
            print("✅ Manual Deadline Input - 'Database exam Friday'")
            print("✅ Auto-Backup to GitHub - Saves memory automatically") 
            if self.outlook:
                print("✅ Outlook Integration - Calendar and email sync")
            else:
                print("⚠️ Outlook Integration - Setup required")
        else:
            print("❌ Academic engines not available")
        print("=" * 60)
...
<body>
    ...
    # Academic assistant methods
    def _is_academic_request(self, user_input: str) -> bool:
        """Check if request is academic-related"""
        academic_keywords = [
            'syllabus', 'deadline', 'exam', 'quiz', 'assignment', 'homework',
            'essay', 'paper', 'project', 'due', 'calendar', 'email',
            'professor', 'class', 'course', 'study', 'upcoming', 'schedule'
        ]
        return any(keyword in user_input.lower() for keyword in academic_keywords)
    
    def _handle_academic_request(self, user_input: str) -> str:
        """Handle all academic requests"""
        try:
            user_lower = user_input.lower()
            
            # Syllabus upload/parsing
            if 'syllabus' in user_lower and ('upload' in user_lower or 'parse' in user_lower):
                return "📄 Drop your syllabus PDF here (drag and drop or paste the full path)"
            
            # Handle PDF file path
            if user_input.strip().endswith('.pdf') and os.path.exists(user_input.strip()):
                try:
                    result = self.syllabus_parser.parse_pdf(user_input.strip())
                    saved = self.syllabus_parser.save_to_memory(result)
                    
                    # Auto-backup after parsing
                    if self.backup:
                        self.backup.backup(f"Added syllabus: {result['course']['name']}")
                    
                    return f"✅ **Parsed {result['course']['name']}**\n" \
                           f"👨‍🏫 Professor: {result['course']['professor']}\n" \
                           f"🕒 Office Hours: {result['office_hours']}\n" \
                           f"📅 Found {len(result['deadlines'])} deadlines\n" \
                           f"💾 Saved {saved} high-confidence deadlines to memory"
                except Exception as e:
                    return f"❌ Failed to parse syllabus: {str(e)}"
            
            # Manual deadline input
            deadline_keywords = ['exam', 'quiz', 'assignment', 'homework', 'essay', 'paper', 'project', 'due']
            if any(kw in user_lower for kw in deadline_keywords):
                # Check if it's a query or an input
                query_words = ['when', 'what', 'show', 'list', 'tell', 'upcoming']
                if not any(qw in user_lower for qw in query_words):
                    result = self.manual_academic.add_deadline(user_input)
                    if result['success']:
                        # Auto-backup after adding deadline
                        if self.backup:
                            self.backup.backup(f"Added deadline: {result['item']}")
                        
                        time_str = f" at {result['time']}" if result.get('time') else ""
                        return f"✅ **Saved: {result['item']}** on {result['date']}{time_str}"
            
            # Show upcoming deadlines
            if any(phrase in user_lower for phrase in ['what\'s due', 'upcoming', 'deadlines', 'what\'s coming']):
                deadlines = self.manual_academic.get_upcoming_deadlines(days=14)
                if deadlines:
                    response = "📅 **UPCOMING DEADLINES:**\n\n"
                    for d in deadlines:
                        days = d['days_until']
                        urgency = "🚨" if days == 0 else "⚠️" if days <= 2 else "📌"
                        time_str = f" at {d['time']}" if d.get('time') else ""
                        response += f"{urgency} {d['date']}{time_str}: **{d['item']}** ({d['course']})\n"
                    return response
                else:
                    return "No upcoming deadlines in the next 2 weeks"
            
            # Calendar/email requests
            if 'calendar' in user_lower or ('email' in user_lower and 'check' in user_lower):
                if self.outlook:
                    try:
                        if 'calendar' in user_lower:
                            events = self.outlook.get_calendar_events(days_ahead=7)
                            if events:
                                response = "📅 **THIS WEEK'S CALENDAR:**\n\n"
                                for event in events[:10]:  # Show first 10
                                    start_date = event['start']['dateTime'].split('T')[0]
                                    start_time = event['start']['dateTime'].split('T')[1][:5]
                                    response += f"📍 {start_date} at {start_time}: **{event['subject']}**\n"
                                    if event.get('location', {}).get('displayName'):
                                        response += f"   📍 {event['location']['displayName']}\n"
                                return response
                            else:
                                return "No calendar events found for the next week"
                            ...
                            if emails:
                                ...
                                for email in emails[:5]:  # Show first 5
                                    ...
                                    response += f"{read_status} **{email['subject']}**\n"
                                    response += f"   From: {from_addr}\n"
                                    response += f"   Preview: {email['bodyPreview'][:100]}...\n\n"
                                return response
                            else:
                                return "No recent emails found"
                    except Exception as e:
                        return f"⚠️ Outlook sync error: {str(e)}\nTry: Check your authentication"
                else:
                    return "⚠️ Outlook integration not available. Check setup."
            
            # Show all deadlines
            if 'all deadlines' in user_lower or 'list deadlines' in user_lower:
                deadlines = self.manual_academic.list_all_deadlines()
                if deadlines:
                    response = "📋 **ALL DEADLINES:**\n\n"
                    for d in deadlines:
                        time_str = f" at {d['time']}" if d.get('time') else ""
                        response += f"📌 {d['date']}{time_str}: **{d['item']}** ({d['course']})\n"
                    return response
                else:
                    return "No deadlines found in memory"
            ...
            if 'backup' in user_lower:
                if self.backup:
                    ...
                    if status.get('initialized'):
                        ...
                        response += f"✅ Git initialized\n"
                        response += f"📄 Files tracked: {status['files_tracked']}\n"
                        response += f"📝 Last commit: {status['last_commit']}\n"
                        if status.get('has_uncommitted_changes'):
                            response += "⚠️ Has uncommitted changes"
                        else:
                            response += "✅ All changes committed"
                        return response
                    else:
                        return "Setting up backup system..."
                else:
                    return "Backup system not available"
            
            # General academic help
            return "🎓 **Academic Assistant Ready!**\n\n" \
                   "📄 Upload syllabus: 'upload syllabus'\n" \
                   "📅 Add deadline: 'Database exam Friday at 2pm'\n" \
                   "🔍 Check deadlines: 'what's due this week'\n" \
                   "📧 Check email: 'check email'\n" \
                   "📆 Check calendar: 'show calendar'"
                   
        except Exception as e:
            return f"❌ Academic request error: {str(e)}"
